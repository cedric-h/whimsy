...
impl super::Lang {
    ...
    // Calls py_draw_commands internally, but formats the error all nice for ya
    pub fn draw_commands(&self) -> Result<std::sync::mpsc::Receiver<DrawCommand>, String> {
        self.py_draw_commands()
            .map_err(|e| Self::format_err(&self.vm, e))
    }

    fn py_draw_commands(&self) -> vm::pyobject::PyResult<std::sync::mpsc::Receiver<DrawCommand>> {
        ...
        let (draws_tx, draws_rx) = std::sync::mpsc::channel();
        ...
        expose_fn!(
            ...
            move |first: Either<PyStringRef, Num>,
                ...
                d.send(Fill(match first {
                    Either::A(color_name) => {
                        colors.get(color_name.as_str()).copied().ok_or_else(|| {
                            vm.new_lookup_error(format!("Unknown color `{}`", color_name))
                        })
                    }
                    Either::B(r) => {
                        let fields =
                            vec![Some(r), g.into_option(), b.into_option(), a.into_option()]
                                .into_iter()
        ...
        expose_fn!("wave", _d, move |vm: &vm::VirtualMachine,
                                     args: vm::function::PyFuncArgs|
              -> vm::pyobject::PyResult {
            use vm::pyobject::{IntoPyObject, TryFromObject};
        ...
        expose_fn!("move", d, move |x: Num, y: Num| {
            d.send(Transform(Matrix::translate((
                x.to_f64() as f32,
                y.to_f64() as f32,
            ))))
            .unwrap();
        ...
        expose_fn!("spin", d, move |angle: Num| {
            d.send(Transform(Matrix::rotate(angle.to_f64() as f32)))
                .unwrap();
            Ok(())
        })?;
        expose_fn!("zoom", d, move |x: Num, y: OptionalArg<Num>| {
            ...
            d.send(Transform(Matrix::scale((x, y.unwrap_or(x)))))
                .unwrap();
            Ok(())
        })?;
